...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     , imgGray(frame.imgGray), imgMasks(frame.imgMasks)
{
...
Frame::Frame(const std::vector<cv::Mat> &masks, const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF), mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false),
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    imgGray = imGray;
    imgMasks = masks;
    for (int i = 0; i < imgMasks.size(); ++i)
        cv::dilate(imgMasks[i], imgMasks[i], cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3)), cv::Point(-1, -1), 5);
/*
    if (mpPrevFrame && mpPrevFrame->N)
    {
        std::vector<cv::Point2f> pt1, pt2;
        for (auto &kp : mpPrevFrame->mvKeys) pt1.push_back(kp.pt);
        std::vector<uchar> status;
        std::vector<float> error;
        cv::Mat prevImgBGR, currentImgBGR;
        cvtColor(mpPrevFrame->imgGray, prevImgBGR, cv::COLOR_GRAY2BGR);
        cvtColor(imgGray, currentImgBGR, cv::COLOR_GRAY2BGR);
        cv::calcOpticalFlowPyrLK(prevImgBGR, currentImgBGR, pt1, pt2, status, error, cv::Size(15, 15), 3);
        // cv::calcOpticalFlowPyrLK(mpPrevFrame->imgGray, imgGray, pt1, pt2, status, error, cv::Size(15, 15), 3);
        std::vector<cv::Point2f> pt1Temp, pt2Temp;
        std::vector<uchar> statusTemp;
        for (int i = 0; i < pt1.size(); ++i)
        {
            bool flag = false;
            for (int j = 0; j < mpPrevFrame->imgMasks.size(); ++j)
            {
                if (255 == mpPrevFrame->imgMasks[i].at<cv::Vec3b>(pt1[i].y, pt1[i].x)[0])
                {
                    flag = false;
                    break;
                }
            ...
            if (!flag)
            {
                pt1Temp.push_back(pt1[i]);
                pt2Temp.push_back(pt2[i]);
                statusTemp.push_back(status[i]);
            }
        }
        std::vector<cv::KeyPoint> keypoints1WithNoPrior, keypoints2WithNoPrior;
        std::vector<int> vMatches12;
        for (int i = 0; i < pt2Temp.size(); ++i)
        {
            bool flag = false;
            for (int j = 0; j < imgMasks.size(); ++j)
            {
                if (255 == imgMasks[i].at<cv::Vec3b>(pt2Temp[i].y, pt2Temp[i].x)[0])
                {
                    flag = false;
                    break;
                }
            ...
            if (!flag)
            {
                keypoints1WithNoPrior.push_back(cv::KeyPoint(pt1Temp[i], 0));
                keypoints2WithNoPrior.push_back(cv::KeyPoint(pt2Temp[i], 0));
                0 == statusTemp[i] ? vMatches12.push_back(-1) : vMatches12.push_back(vMatches12.size());
            }
        }
        ORB_SLAM3::TwoViewReconstruction tvr(mK_);
        Sophus::SE3f T21;
        ...
        float score;
        Eigen::Matrix3f F21;
        tvr.Reconstruct(keypoints1WithNoPrior, keypoints2WithNoPrior, vMatches12, T21, vP3D, vbTriangulated);
        tvr.FindFundamental(vbMatchesInliers, score, F21);
        std::vector<cv::KeyPoint> keypoints2;
        for (int i = 0; i < pt2.size(); i++)
            keypoints2.push_back(cv::KeyPoint(pt2[i], 0));
        tvr.Reconstruct(mpPrevFrame->mvKeys, keypoints2, vMatches12, T21, vP3D, vbTriangulated);
        tvr.CheckFundamental(F21, vbMatchesInliers, tvr.mSigma);
        std::vector<cv::Mat> dynamicMask;
        for (auto &mask : imgMasks)
        {
            std::vector<int> idxMask;
            for (int i = 0; i < pt2.size(); ++i)
                if (255 == mask.at<cv::Vec3b>(pt2[i].y, pt2[i].x)[0])
                    idxMask.push_back(i);
            int outlierCnt = 0, inlierCnt = 0;
            for (auto idx : idxMask)
                if (status[idx])
                    for (int i = 0; i < vbMatchesInliers.size(); ++i) 
                        if (tvr.mvMatches12[i].second == idx)
                            if (vbMatchesInliers[i]) ++inlierCnt;
                            else ++outlierCnt;
            if ((double)outlierCnt / (inlierCnt + outlierCnt) > 0.1)
            {
                dynamicMask.push_back(mask);
                cout << (double)outlierCnt / (inlierCnt + outlierCnt) << endl;
            }
        }
        std::vector<std::vector<int> > dynamicKpsIndex;
        for (int i = 0; i < dynamicMask.size(); ++i)
        {
            std::vector<int> tmp;
            for (int j = 0; j < mvKeys.size(); ++j)
                if (255 == dynamicMask[i].at<cv::Vec3b>(mvKeys[j].pt.y, mvKeys[j].pt.x)[0])
                    tmp.push_back(j);
            dynamicKpsIndex.push_back(tmp);
        }
        std::set<int> needToDelete;
        for (auto iter : dynamicKpsIndex)
        {
            std::set<int> tmp(iter.begin(), iter.end());
            std::set<int> tmp1 = needToDelete;
            std::set_union(tmp1.begin(), tmp1.end(), tmp.begin(), tmp.end(), inserter(needToDelete, needToDelete.begin()));
        }
        for (auto iter=needToDelete.rbegin(); iter!=needToDelete.rend(); ++iter)
        {
            mvKeys.erase(mvKeys.begin() + *iter);
            if (*iter != mDescriptors.rows-1)
                mDescriptors.rowRange(*iter+1, mDescriptors.rows).copyTo(mDescriptors.rowRange(*iter, mDescriptors.rows-1));
            mDescriptors.resize(mDescriptors.rows-1);
        }
    }
*/
    std::vector<std::vector<int> > priorDynamicKpsIndex;
    for (int i = 0; i < imgMasks.size(); ++i)
    {
        std::vector<int> tmp;
        for (int j = 0; j < mvKeys.size(); ++j)
            if (255 == imgMasks[i].at<uchar>(mvKeys[j].pt.y, mvKeys[j].pt.x))
                tmp.push_back(j);
        priorDynamicKpsIndex.push_back(tmp);
    }
    std::set<int> needToDelete;
    for (auto iterPrior : priorDynamicKpsIndex)
    {
        std::set<int> tmp(iterPrior.begin(), iterPrior.end());
        std::set<int> tmp1 = needToDelete;
        std::set_union(tmp1.begin(), tmp1.end(), tmp.begin(), tmp.end(), inserter(needToDelete, needToDelete.begin()));
    }
    for (auto iter=needToDelete.rbegin(); iter!=needToDelete.rend(); ++iter)
...
Frame::Frame(const std::vector<cv::Mat> &masks, const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(static_cast<Pinhole*>(pCamera)->toK()), mK_(static_cast<Pinhole*>(pCamera)->toK_()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL),mpPrevFrame(pPrevF),mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false), mpCamera(pCamera),
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
