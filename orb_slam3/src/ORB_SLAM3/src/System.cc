...
Sophus::SE3f System::TrackRGBD(const vector<cv::Mat> &masks, const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if(mSensor!=RGBD  && mSensor!=IMU_RGBD)
    {
        cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(masks, imToFeed,imDepthToFeed,timestamp,filename);
...
Sophus::SE3f System::TrackMonocular(const vector<cv::Mat> &masks, const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(masks, imToFeed,timestamp,filename);
